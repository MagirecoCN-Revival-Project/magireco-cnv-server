...
```json
{
  ...
  "asset_auth": { "type": "bearer", "token": "...", "expires_at": 1785090420000 },
...
```json
{ "type": "bearer", "token": "...", "expires_at": 1785090420000 }
```

`type` 是**判别字段**,其余字段的形状由它决定。当前唯一取值 `bearer`,承载既有的
`resource_token`(HMAC 签名、绑设备、按时间窗轮换)。
...
服务端侧有一条对应的 fail-closed:签名密钥过短时**不下发 `asset_auth`**,而不是
用空密钥算一个令牌。空密钥的 HMAC 照样能算出"看起来正常"的令牌,而那个令牌
任何人都能自己算出来,等于把资产完全敞开。
